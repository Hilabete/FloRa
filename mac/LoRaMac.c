...
{
		printf("LoRaMacSendFrame( fPort, fBuffer, fBufferSize )\n");
		uint8_t result = 0;
    LoRaMacHeader_t macHdr;
    ...
    macHdr.Bits.MType = FRAME_TYPE_DATA_UNCONFIRMED_UP;
		result = LoRaMacSend( &macHdr, NULL, fPort, fBuffer, fBufferSize );
		printf("LoRaMacSendFrame return %d\n", result);
    return result;
}
...
{
		printf("LoRaMacSend( *macHdr, *fOpts, fPort, *fBuffer, fBufferSize )\n");
		uint8_t result = 0;
    ...
    if( LoRaMacSetNextChannel( ) == 0 )
    {		
				result = LoRaMacSendOnChannel( Channels[Channel], macHdr, &fCtrl, fOpts, fPort, fBuffer, fBufferSize );
				printf("LoRaMacSend return %d\n", result);
        return result;
    }
    return 5;
}
...
uint8_t LoRaMacSendOnChannel( ChannelParams_t channel, LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t *fOpts, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    LoRaMacEventInfo.TxAckReceived = false;
		...
		result = LoRaMacSendFrameOnChannel( channel );
		printf("LoRaMacSendOnChannel %d\n", result);
    return result;
}
